...
//supplementary code to KinDEG_1_0
//adapted from line graph example by Christian Marc Schmidt
...
PFont comicSans;
char choice = 'c';
String logChoice = "C:/Users/Wei Jian/Documents/GitHub/KinDEG/Experimental Versions/KINDEG_0_3/totalUserLog.txt";
String totalUserLog = "C:/Users/Wei Jian/Documents/GitHub/KinDEG/Experimental Versions/KINDEG_0_3/totalUserLog.txt";
String engagedUsersLog = "C:/Users/Wei Jian/Documents/GitHub/KinDEG/Experimental Versions/KINDEG_0_3/engagedVisitorsLog.txt";
  ...
  switch(choice)
  {
    case 'y': //sort by year
      ...
      {   
        ...
        if (isIn(xaxis,(temp.gety()))!=1)
        {//if not in xaxis already, put it in   
          xaxis.add(temp.y);
    ...
    case 'm': //sort by month
      ...
      {
        temp = (timeEl)dateList.get(i);
        if (isIn(xaxis,temp.mon)==0)
        {//if not in xaxis already
          //put it in
          xaxis.add(temp.mon);
          yaxis.add(temp.parameter);     
        }
    ...
    case 'd': //sort by day/date
        ...
        {
          temp = (timeEl)dateList.get(i);
          if (isDateIn(dates,temp)==0)
          {//if date has not appeared, put it in
            token++;
            dates.add(temp);
            xaxis.add(token);
            yaxis.add(temp.parameter);   
  ...
  for (int i = 0; i < arry.length; i++) {
    ...
    if ((delta < 2) && (y > plotY1) && (y < plotY2)) 
    {
      ...
      if (choice == 'c')
      {
        ...
        String cLabelPart2;
        String cLabelPart3;
        if (hourPart.charAt(0) != '0')
        {
          cLabelPart1 = str(arrx[i]).substring(0,2);   
          cLabelPart2 = str(arrx[i]).substring(2,4);
          cLabelPart3 = str(arrx[i]).substring(4,6); 
        }
        else
        {
          cLabelPart1 = str(arrx[i]).substring(0,1);   
          cLabelPart2 = str(arrx[i]).substring(1,3);
          cLabelPart3 = str(arrx[i]).substring(3,5); 
        }
