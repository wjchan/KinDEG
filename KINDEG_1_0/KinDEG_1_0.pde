...
int maxUserNum = 0;
HashMap userDurationMap = new HashMap();
...
timer timerLog = new timer(10000); 
timer timerSecond = new timer(500);
...
void setup()
{
 ...
 //serial stuff 
  ...
  opencv.cascade("C:/opencv/data/haarcascades/","haarcascade_frontalface_alt2.xml"); //select descriptor
}
...
void draw()
{
  ...
  if (timerSecond.timesUp())
  {
    if (camEnabled)
    {
      takeVid(); //take a pic every second if cam is enabled
    }
    timerSecond.restart();
  }
  ...
  {
    maxUserNum = userList.length; //get maxUser1Time
  }
  ...
  for(int i=0;i<userList.length;i++)
  {
    if(kinect.isTrackingSkeleton(userList[i]))
    {
      ...
      float[] coord = getHeadCoord(userList[i]);
         if (camEnabled) //take pic if cam is enabled
          {
           takePic();
          }
      if (coord[2]>0.5) //if confidence that head joint is there > 0.5
      { 
        if (serialEnabled) //send message to arduino if serial is enabled
        {
...
void logIt(boolean last){ //put into log files
  ...
  //LOG STUFF
  //declare variables to be placed in log file
